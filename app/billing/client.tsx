...
type Summary = {
  ...
  byZone: { zone: string; cost: number }[]; krwPerUsd: number; byProject: { id: number; name: string; budget_usd: number; cost: number }[]; recent: { id: number; ts: string; agent_name: string; model: string; mode: string; input_tokens: number; cached_tokens: number; output_tokens: number; cost_usd: number }[];
...
const krw = (usd: number, rate: number) => `₩${Math.round(usd * rate).toLocaleString("ko-KR")}`;
const Money = ({ usd, rate, small }: { usd: number; rate: number; small?: boolean }) => <span className="money">${usd.toFixed(2)}<em>{krw(usd, rate)}</em>{small}</span>;
...
export function BillingClient() {
  ...
  return (
    <div>
      ...
      <div className="tiles">
        <div className="tile"><div className="l">잔여 크레딧</div><div className="v"><Money usd={d.creditsRemaining} rate={d.krwPerUsd} /> <small>/ ${d.creditsTotal.toFixed(0)} 충전</small></div><div className="gauge"><i className={d.creditsRemaining < 20 ? "bad" : ""} style={{ width: `${d.creditsTotal ? Math.max(0, Math.min(100, d.creditsRemaining / d.creditsTotal * 100)) : 0}%` }} /></div><div className="sub">{d.creditsTotal === 0 ? "충전 기록 없음" : d.creditsRemaining < 20 ? "충전 필요" : "정상"}</div></div>
        <div className="tile"><div className="l">오늘</div><div className="v"><Money usd={d.today} rate={d.krwPerUsd} /></div><div className="sub">산출물 {d.outputsToday}건{!d.liveMode && " · 시뮬레이션"}</div></div>
        <div className="tile"><div className="l">이번 달</div><div className="v"><Money usd={d.month} rate={d.krwPerUsd} /> <small>/ 한도 ${d.limit.monthly_usd}</small></div><div className="gauge"><i className={pctUsed >= 100 ? "bad" : pctUsed >= d.limit.alert_pct ? "warn" : ""} style={{ width: `${pctUsed}%` }} /></div><div className="sub">실비 ${d.monthLive.toFixed(2)} · {d.limit.alert_pct}%에서 알림</div></div>
        <div className="tile"><div className="l">산출물 1건당</div><div className="v"><Money usd={d.outputsToday ? d.today / d.outputsToday : 0} rate={d.krwPerUsd} /></div><div className="sub">오늘 평균 · 환율 ₩{d.krwPerUsd.toLocaleString()}/$ 기준</div></div>
      </div>
      <div className="grid g2" style={{ marginTop: 14 }}>
        ...
        <div className="card zones"><b>이번 달 존별</b><div className="muted" style={{ marginBottom: 8 }}>이미지 생성은 젠스파크 핸드오프 — API 비용 0</div>
          {(() => { const mx = Math.max(0.01, ...d.byZone.map((x) => x.cost)); return ["design", "growth", "sales", "reception", "image"].map((z) => { const v = z === "image" ? 0 : (d.byZone.find((x) => x.zone === z)?.cost || 0); return (
            <div className="zrow" key={z}><span className="zn">{z === "image" ? "이미지" : ZONE_NAME[z]}</span><span className="zbar"><i style={{ width: `${Math.max(v > 0 ? 3 : 0, (v / mx) * 100)}%`, background: v > 0 ? ZONE_COLOR[z] : "var(--line-strong)" }} /></span><span className="zv mono">${v.toFixed(2)}</span><span className="zk mono">{krw(v, d.krwPerUsd)}</span></div>); }); })()}
        </div>
      </div>
      <div className="scroll" style={{ marginTop: 14 }}>
        <table><thead><tr><th>담당자</th><th className="num">오늘 호출</th><th className="num">입력 토큰</th><th className="num">출력 토큰</th><th className="num">오늘 비용</th><th className="num">실비</th><th className="num">일일 한도</th></tr></thead>
          <tbody>{d.byAgent.map((a) => <tr key={a.agent_id} className={a.calls === 0 ? "zero" : ""}><td>{a.name} <span className="muted">{a.role_title}</span></td><td className="num">{a.calls}</td><td className="num">{a.input_tokens.toLocaleString()}</td><td className="num">{a.output_tokens.toLocaleString()}</td><td className="num">${a.cost.toFixed(2)} <span className="muted">{krw(a.cost, d.krwPerUsd)}</span></td><td className="num">${a.live_cost.toFixed(2)}</td><td className="num">${a.daily_cost_cap}</td></tr>)}</tbody></table>
      </div>
