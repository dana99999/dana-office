...
export default async function Archive() {
  ...
  return (<div style={{ maxWidth: 920 }}><div className="topbar"><h1>아카이브</h1><span className="sub">전체 지시 {items.length}건 · 담당자 산출물을 PM이 종합해 보관</span></div>
    ...
    <div className="grid">{items.map((d) => { const st = ST[d.status] || ST.open; return (
      <div className="card" key={d.id}>
        <div className="row" style={{ justifyContent: "space-between", marginBottom: 8 }}><div><b style={{ fontSize: 15 }}>#{d.id} {d.body}</b><div className="muted">{d.requester_name} · {d.project_name} · {d.created_at.slice(0, 16)}</div></div><span className={`chip ${st[0]}`}>{st[1]}</span></div>
        <div className="lhead">참여 담당자</div>
        ...
        {d.summary_artifact_id ? <Link href={`/artifacts/${d.summary_artifact_id}`} className="btn sm primary">종합 정리 보기{d.summary_title ? ` — ${d.summary_title}` : ""}</Link> : <span className="muted">종합 정리는 담당자 산출물이 모두 들어온 뒤 PM이 작성합니다.</span>}
      </div>); })}</div></div>);
}
